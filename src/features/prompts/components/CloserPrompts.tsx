...
const TopSideButtons = ({ applySearch }: PropTypes) => {
  ...
  const openAddNewPromptModal = () => {
    dispatch(openModal({ title: "Add New Prompt", bodyType: MODAL_BODY_TYPES.CLOSER_PROMPT_ADD_NEW }))
  }
...
function CloserPrompts() {
  ...
  const deleteCurrentPrompt = (id: string) => {
    dispatch(openModal({
      ...
      extraObject: { message: `Are you sure you want to delete this prompt?`, type: CONFIRMATION_MODAL_CLOSE_TYPES.CLOSER_PROMPT_DELETE, id }
    }))
  }

  const editCurrentPrompt = (prompt: Prompt) => {
    dispatch(openModal({ title: "Edit Prompt", bodyType: MODAL_BODY_TYPES.CLOSER_PROMPT_UPDATE, extraObject: prompt }))
  }
